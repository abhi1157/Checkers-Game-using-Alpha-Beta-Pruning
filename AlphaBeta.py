...
'''
I have used some heuristic to make alpha beta work fast and reduce the number of states generated by large amount
'''
def alphbeta(lis, color, movesRemaining):
    ...
    for move in moves:
        ...
        moveVal = MIN_VALUE(newlis, color, depth,opcol, alpha, beta)
...
def MAX_VALUE(lis,color,depth,opcol,alpha,beta):
    ...
    if depth > 1 :
        ...
        for move in moves:
            ...
            if opti==None or (beta!=None and beta > opti):
                value = MIN_VALUE(nextlis, color, depth, opcol, alpha, beta)
                if opti==None or (value!=None and value > opti):
                    opti = value
                if alpha==None or(opti!=None and opti > alpha):
                    alpha = opti
        return opti
    else:
            utility=Utility(lis,color,opcol)
            return utility
def MIN_VALUE(lis,color,depth,opcol,alpha,beta):
    ...
    if depth > 1 and Terminal_test(lis)==False:
        ...
        for move in moves:
            nextlis = deepcopy(lis)
            general_func.doMove(nextlis,move)
            if alpha == None or opti == None or alpha < opti:
                value = MAX_VALUE(nextlis, color, depth,opcol, alpha, beta)
                if opti == None or  (value!=None and value < opti):
                    opti = value
                if opti==None or (beta!=None and opti < beta):
                    beta = opti
        return opti
    else:
            utility=Utility(lis,color,opcol)
